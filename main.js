...
grid.addEventListener("input", (e) => {
  ...
  const diff = newQty - owned[item.name];
  if (diff > 0 && money >= item.price * diff) {
    owned[item.name] = newQty;
    money -= item.price * diff;
  } else if (diff < 0 && newQty >= 0) {
    owned[item.name] = newQty;
    money += item.price * Math.abs(diff);
  } else {
    input.value = owned[item.name];
  }
  updateUI();
});
